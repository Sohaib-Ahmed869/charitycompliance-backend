...
export class OnboardingProgressRepository {
  ...
  async recalculateProgress(orgId) {
    ...
    // Calculate full profile completion (all steps including initial + ACNC steps)
    const fullProfileSteps = [
      ...initialSteps,
      progress.abn_verification_complete,
      progress.legal_structure_complete,
      progress.org_details_complete,
      progress.responsible_people_complete,
      progress.registration_date_complete,
      progress.operating_locations_complete,
      progress.activities_complete,
      progress.subtypes_complete,
      progress.finances_complete,
      progress.governance_complete,
      progress.tax_complete,
      progress.withhold_info_complete,
      progress.documents_complete,
      progress.authorised_contact_complete,
      progress.declaration_complete,
      progress.platform_setup_complete
    ];
    const completedFullProfileSteps = fullProfileSteps.filter(Boolean).length;
    const fullProfileCompletionPercentage = Math.round((completedFullProfileSteps / 20) * 100);
